...
static void pcm_irq(void *arg)
{
  ...
  if (g_pcm.status.capture)
  ...
  else
    {
      ...
      count = bk7258_aud_dac_write(block, available ? available : PCM_IRQ_BUDGET);
      if (available != 0)
        {
...
{
  if (!g_pcm.active || session == 0 || session != g_pcm.session) return -EBADF;
  return 0;
}
...
int bk7258_pcm_write(uint32_t session, const int16_t *samples, uint32_t count)
{
  ...
  for (i = 0; i < count; i++)
    {
      ...
      if (value > 1024) { value = 1024; limited++; }
      if (value < -1024) { value = -1024; limited++; }
      if (ramp < PCM_RAMP)
