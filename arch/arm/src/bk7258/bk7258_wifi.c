...
int bk7258_wifi_initialize(void)
{
  ...
  _Static_assert(sizeof(CONFIG_BK7258_WIFI_IFNAME) > 3 &&
...
{
  struct bk7258_wifi_profile_request request = {0};
  struct bk7258_wifi_profile_response response = {0};
  uint16_t received = 0;
  int ret;
  ...
  if (operation == BK7258_WIFI_PROFILE_SET)
    {
      if (!bk7258_wifi_profile_valid(profile)) return -EINVAL;
...
int bk7258_wifi_random_block(void *buffer, size_t length)
{
  ...
  _Static_assert(sizeof(response) == 40, "CP TRNG ABI size");
  ...
  ret = wifi_command(0x213u, NULL, 0, &response, sizeof(response), &received);
